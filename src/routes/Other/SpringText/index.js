...
// MAIN
    ...
    function Point(x,y,r,g,b,a) {
      ...
      this.checkMove = function() {
         this.moved = (!Math.abs(Math.round(this.target_x - this.x)) == 0 || !Math.abs(Math.round(this.target_y - this.y)) == 0 || !Math.abs(Math.round(this.velx)) == 0 || !Math.abs(Math.round(this.vely)) == 0); //eslint-disable-line
      }
    ...
    function adjustText() {
      ...
      if (textWidth == 0) {//eslint-disable-line
        return;
      }
      ...
      for (var i=0; i<data.length; i++) {
        if (data[i] != 0) {//eslint-disable-line
          isBlank = false;
          break;
        }
      }
      ...
      if (!isBlank) {
        ...
        POINTS = [];
        while (count < AMOUNT) {
          while (curr == 0) {//eslint-disable-line
    ...
    function draw() {
      ...
      for (var i=0; i<POINTS.length; i++) { //eslint-disable-line
        POINTS[i].draw()
      }
